-- ============================================================
-- Radar de Licitações — schemas.sql (Supabase / PostgreSQL)
-- ============================================================

-- ------------------------------------------------------------
-- 1. Function + trigger de updated_at (reutilizada por todas)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 2. Tabelas sem FK para outras tabelas de negócio primeiro
-- ------------------------------------------------------------
...
-- === sources (global, mantida pelo backend) ===
...
create unique index idx_sources_slug on public.sources (slug);
...
-- sources: leitura para authenticated; escrita apenas service_role (ignora RLS)
create policy "sources_select_authenticated" on public.sources
  for select to authenticated using (true);

-- ------------------------------------------------------------
-- 3. Tabelas com FK para as anteriores
-- ------------------------------------------------------------
...
create policy "product_suppliers_select_own" on public.product_suppliers
  for select to authenticated using (owner_id = auth.uid());
create policy "product_suppliers_insert_own" on public.product_suppliers
  for insert to authenticated with check (owner_id = auth.uid());
create policy "product_suppliers_update_own" on public.product_suppliers
  for update to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "product_suppliers_delete_own" on public.product_suppliers
  for delete to authenticated using (owner_id = auth.uid());
...
-- opportunities: leitura pelo dono; escrita apenas service_role (varredura)
...
-- leitura pelo dono via join; escrita service_role
create policy "opportunity_products_select_own" on public.opportunity_products
...
create policy "edital_analyses_select_own" on public.edital_analyses
  for select to authenticated using (
    exists (
      select 1 from public.opportunities o
      where o.id = edital_analyses.opportunity_id
        and o.owner_id = auth.uid()
    )
  );
...
create trigger trg_financial_simulations_updated_at
  before update on public.financial_simulations
  for each row execute function public.set_updated_at();

alter table public.financial_simulations enable row level security;

create policy "financial_simulations_select_own" on public.financial_simulations
  for select to authenticated using (owner_id = auth.uid());
create policy "financial_simulations_insert_own" on public.financial_simulations
  for insert to authenticated with check (owner_id = auth.uid());
create policy "financial_simulations_update_own" on public.financial_simulations
  for update to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "financial_simulations_delete_own" on public.financial_simulations
  for delete to authenticated using (owner_id = auth.uid());
...
create trigger trg_participation_history_updated_at
  before update on public.participation_history
  for each row execute function public.set_updated_at();

alter table public.participation_history enable row level security;

create policy "participation_history_select_own" on public.participation_history
  for select to authenticated using (owner_id = auth.uid());
create policy "participation_history_insert_own" on public.participation_history
  for insert to authenticated with check (owner_id = auth.uid());
create policy "participation_history_update_own" on public.participation_history
  for update to authenticated using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "participation_history_delete_own" on public.participation_history
  for delete to authenticated using (owner_id = auth.uid());
...
-- notifications: criadas pelo backend (service_role); dono lê, marca lida e apaga
...
-- sync_logs: escritos pelo backend (service_role); qualquer authenticated lê p/ auditoria
